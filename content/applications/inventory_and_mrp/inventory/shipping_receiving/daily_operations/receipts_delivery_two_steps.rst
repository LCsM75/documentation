...
When products are received in two steps, they move from the vendor location to an input location,
then from the input location to warehouse stock in the database upon validation of a purchase order
...
.. tip::
   ...
   :guilabel:`Deliver To` field on the |PO| form might need to be specified as the correct *input
...
When products are delivered in two steps, they move from warehouse stock to an output location, then
from the output location to a customer location in the database upon validation of a picking order,
...
task card on the :guilabel:`Inventory Overview` dashboard.

Click the :guilabel:`# To Process` button on the task card. This reveals a list of all pickings to
process.
